...
class EmpresaPruebasDetalleController extends Controller
{
    public function index(Request $request){
       ...
       return view('admin.empresa_pruebas_detalle' , ['empresa_pruebas_detalle' => $empresas_arr, 'test' => $empresaTestAll, 'empresas' => $empresasAll, 'numsec_prueba' => $request["id"],  'titulo' => $titulo, '_user_'  => $this->getUserData()]);
   }
   ...
   public function store(Request $request)
    {
       if($request['con_detpru'] !== null){
         $update = \DB::table('empresa_pruebas_detalle') ->where('con_detpru', $request['con_detpru']) ->limit(1) ->update( [ 'con_test' => $request['con_test']]);  
       }
       else{
         $empresas = EmpresaPruebasDetalle::create($request->all());  
       }

 
        return Redirect::to('admin/empresa_pruebas_detalle/'.$request['numsec_prueba']);
    ...
    {
        DB::table('empresa_pruebas_detalle')->where('con_detpru', '=', $id)->delete();
